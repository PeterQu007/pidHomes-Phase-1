...
if (function_exists('putRevSlider') && (!empty($rev_slider_alias))) {
    ...
    $banner_image_id = get_field('page_banner_background_image')['sizes']['pageBanner-about'];
    // print_X('orange', 'banner image', $banner_image_id); //d//
    if ($banner_image_id) {
        //:: $banner_image_path = wp_get_attachment_url($banner_image_id);
        $banner_image_path = $banner_image_id;
    } else {
        $banner_image_path = get_default_banner();
    }
    //echo $banner_image_path;
    ...
    ?>
	<section class="rh_banner rh_banner__image-about" 
      style="background-repeat: no-repeat;background-position: center top; 
        background-image: url('<?php echo esc_url($banner_image_path); ?>'); background-size: cover; ">
